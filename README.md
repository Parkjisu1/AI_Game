...
| 자동화 스크립트 | 17개 Node.js + 4개 공유 라이브러리 |
| 커스텀 에이전트 | 8종 |
| 슬래시 커맨드 | 8종 |
...
```
        ...
        │  (Sonnet)  │ │  Coder  │  │  │ Coder×2 │  │  Coder      │
        │  기획 전문  │ │ (Opus)  │  │  │(Sonnet) │  │  (Sonnet)   │
        └────────────┘ │ Core+핵심│  │  │ 병렬 구현│  │  HTML5 광고  │
                       └─────────┘  │  └─────────┘  └─────────────┘
                              ┌─────┴──────┐   ┌──────────────┐
                              │  Validator  │   │  DB Builder  │
                              │  (Sonnet)   │   │  (Sonnet)    │
                              │  QA 검증    │   │  호출형      │
                              └────────────┘   └──────────────┘
```
...
| Agent | Model 선택 이유 | 역할 범위 |
|-------|----------------|----------|
| **Lead** | Opus — 복잡한 의존성 판단, 다중 에이전트 조율 필요 | 태스크 분배만. 직접 코드를 작성하지 않음 (delegate 모드) |
| **Designer** | Sonnet — 기획은 구조화된 YAML 출력이 핵심, 속도 우선 | Layer 1(기획서) → Layer 2(명세서) → Layer 3(YAML 노드) |
| **Main Coder** | Opus — Core 아키텍처 설계는 전체를 이해해야 하므로 추론 능력 우선 | Phase 0 Core 전담 + _ARCHITECTURE.md 생성 + 복잡한 시스템 |
| **Sub Coder** | Sonnet × 2 — Main이 세운 패턴을 따라 구현, 속도·병렬성 우선 | Main의 패턴 준수하여 할당 노드 구현 |
| **Validator** | Sonnet — 검증은 체크리스트 기반, 판단보다 정확성 우선 | 5단계 검증 (Syntax → Dependency → Contract → NullSafety → Pattern) |
| **DB Builder** | Sonnet — 파싱은 규칙 기반 변환, 사용자 호출 시에만 활성화 | C# AST 파싱 → Layer/Genre/Role/Tag 분류 → DB 저장 |
...
| **Designer (design mode)** | 8단계 기획 워크플로우 실행 | 2~6단계 |
| **Design Validator** | 교차 검증, 밸런스 시뮬, 점수 관리 | 3단계, 7단계 |
...
| `score-manager.js` | 신뢰도 점수 계산 라이브러리 |
...
| 2. 유저 여정 시뮬 | Day 1~30 페르소나 기반 진행 추적 |
| 3. 갭 탐지 | 참조 누락, 미정의 아이템, 접근 불가 기능 |
| 4. 자가 검증 주석 | 검증 결과를 기획서에 주석으로 기록 |
...
│
├── .claude/
│   ├── agents/                        # 커스텀 에이전트 정의 (8종)
│   │   ├── designer.md                #   기획 AI
│   │   ├── main-coder.md              #   메인 코드 생성 AI
│   │   ├── sub-coder.md               #   서브 코드 생성 AI
│   │   ├── playable-coder.md          #   HTML5 광고 AI
│   │   ├── validator.md               #   코드 검증 AI
│   │   ├── db-builder.md              #   코드 DB 구축 AI
│   │   ├── design-db-builder.md       #   기획 DB 구축 AI
│   │   └── design-validator.md        #   기획 검증 AI
│   ├── commands/                      # 슬래시 커맨드 (8종)
│   └── skills/                        # 자동 트리거 스킬 (3종)
│
├── db/                                # 데이터베이스
│   ├── base/                          #   Code Base DB (958 파일)
│   │   ├── generic/, rpg/, idle/, puzzle/, ...
│   ├── expert/                        #   Code Expert DB (score >= 0.6)
│   ├── rules/                         #   코드 피드백 규칙
│   ├── design/                        #   Design DB
│   │   ├── base/                      #     Design Base DB (97 파일)
│   │   ├── expert/                    #     Design Expert DB
│   │   └── rules/                     #     기획 피드백 규칙
│   └── ui_meta/                       #   UI 프리팹 메타데이터
│
...
│   └── navigation/                    #   화면 네비게이션
│
├── ai_tester/                         # Cross-genre 게임 테스트 프레임워크
...
│
├── proposal/                          # 기획 문서 & 리뷰
│   ├── AI_workflow_기획.md            #   원본 (v1.1)
│   ├── AI_workflow_기획_v2.0.md       #   완성본 (v2.0)
