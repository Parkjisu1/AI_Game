...
> - `proposal/workflow/AI_workflow_기획_v2.0.md` — 기획 Workflow 정본 (8단계)
...
| **기획서 생성** | 원본 2단계에 포함 (단일 AI) | **독립 Workflow로 분리** (Design Workflow 8단계) |
...
원본에 없던 **완전 신규** 워크플로우. `AI_workflow_기획_v2.0.md`에서 정의.

| 단계 | 명칭 | 핵심 내용 |
|------|------|-----------|
| Stage 1 | Base Design DB 가공 | 기획 자료/AI Tester 관찰 → 정규화 → Design DB 축적 |
...
```
기획 Workflow 독립 실행:
  /generate-design-v2 만으로 Stage 1~6 완료 가능
...
|-----------|----------------|-------------------|-----------|
| 1단계: Base Code DB 가공 | **Phase 1** (DB 구축) | **Stage 1** (Design DB 가공) | 양쪽 각자 DB 구축. 분류 체계 확장 (Layer 3종, Genre 9종, Role 21종) |
| 2단계: 기획서 제작/가공 | **Phase 2** (변환만) | **Stage 2~6** (기획 전체) | **최대 변화**. 원본의 기획서 생성이 독립 Workflow(8단계)로 분리. Code Phase 2는 변환만 담당 |
...
| 10 | Design Workflow 8단계 전체 | Design | 완전 신규 |
...
| 16 | 프로덕션 스크립트 15개 + 라이브러리 4개 | 공통 | 완전 신규 |
| 17 | KPI & 히스토리 관리 | 공통 | 완전 신규 |
...
게임을 만들어줘. Agent Team으로 진행.
  → 기획서가 없으면: 기획 Workflow(Stage 1~6)부터 실행
...
│   │     정의: Design DB, 기획 생성, 검증, AI Tester, Quality Gates, 라이브 동기화
...
docs/
└── WORKFLOW.md                                  ← 시스템 문서 (v2.0 반영 완료)

CLAUDE.md                                        ← 마스터 컨텍스트 (v2.0 반영 완료)
```
...
| `proposal/workflow/AI_workflow_기획_v2.0.md` | 기획 Workflow 전체 사양서 | 기획 Workflow 실행 시 |
...
2. 원본의 2단계 "기획서 제작"이 **AI_Workflow_기획**이라는 독립 8단계 파이프라인으로 확장됨
3. 현재 Code Workflow는 4개 Phase로 재편, 원본 4·5·6단계가 Phase 4로 통합
4. **기획 Workflow는 독립 실행 가능** (Stage 1~6), 코드 Workflow 없이 기획서만 산출 가능
5. **코드 Workflow는 기획서가 필요** — 없으면 기획 Workflow 호출, 있으면 Phase 2부터 진행
6. **Stage 7~8은 공유 단계** — 기획 예측 + 코드 빌드 양쪽이 있어야 실행 가능
7. 원본 대비 **17개 신규 항목**이 추가됨 (Code 9개, Design 6개, 공통 2개)
