...
# v42 (2026-05-22) — n_colors-aware lambda dispatch.
# ══════════════════════════════════════════════════════════

def generate_field_from_metaphor(rows, cols, num_colors, metaphor, level, seed_offset=0, has_life_gimmick=False):
    """메타포 텍스트 → n_colors-aware ZoneMap → 색상 배정 → BL field_map.
    v42: resolve_zone_metaphor 가 n_colors 받아 ring/sector 수 동적 조정.
    같은 메타포라도 num_colors 별로 다른 패턴 → 진짜 다양성.
    Returns (grid, palette_ids): grid[y][x] = BL palette ID (1..28) or 0(T/empty)."""
    try:
    ...
    # v42 잔여 mod10 후처리 — verify_x10.py 의도 정합 (모든 색상 셀 수 = 10배수).
    # assign_colors_x10 의 5-라운드 repair 후에도 일부 메타포·nc 에서 잔여 mod10 가능.
    # 잔여 카운트만큼 그 색의 가장 외곽 가까운 셀을 T(0) 로 변환.
    from collections import Counter
    counts = Counter(v for row in out for v in row if v != 0)
    for pid, cnt in list(counts.items()):
        residue = cnt % 10
        if residue == 0:
            continue
        # 외곽 거리 기준 정렬 (가장 가장자리 우선)
        candidates = []
        for y in range(H):
            for x in range(W_local):
                if out[y][x] == pid:
                    edge_dist = min(y, H - 1 - y, x, W_local - 1 - x)
                    candidates.append((edge_dist, y, x))
        candidates.sort()  # 가장자리 셀 먼저
        for _, y, x in candidates[:residue]:
            out[y][x] = 0  # T 변환

    return out, palette_ids


def is_metaphor_recognized(metaphor):
    """v42 fuzzy 매칭 — _make_zone_map_entry 로 임시 매핑 생성하여 키 검사."""
    if not metaphor:
        return False
    sample_map = _make_zone_map_entry(6)  # 임시 nc=6 으로 키만 확인
    if metaphor in sample_map:
        return True
